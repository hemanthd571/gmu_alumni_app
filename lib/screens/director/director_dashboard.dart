...
class _DirectorDashboardState extends State<DirectorDashboard> {
  ...
  Future<void> _fetchStats() async {
    try {
      ...
      final response = await dio.get('/director/get_stats.php');
      
      if (response.statusCode == 200 && response.data['success']) {
        if (mounted) {
          setState(() {
            _totalAlumni = response.data['total_users'] ?? 0;
        ...
        child: Padding(
          ...
          child: Row(
            ...
            children: [
              ...
              _buildNavIcon(Icons.people_alt_rounded, 'Directory', () => context.push('/director-alumni-directory')),
              _buildNavIcon(Icons.admin_panel_settings_rounded, 'SPOCs', () => context.push('/director-spocs')),
              _buildAddButton(context),
              _buildNavIcon(Icons.check_circle_rounded, 'Approve', () => context.push('/director-approve-posts')),
              _buildNavIcon(Icons.campaign_rounded, 'Notices', () => context.push('/director-noticeboard')),
            ],
          ),
        ),
      ),
        ...
        children: [
          Row(
            children: [
              Container(
                decoration: BoxDecoration(
                  shape: BoxShape.circle,
                  border: Border.all(color: AppConfig.secondaryColor, width: 2),
                ),
                child: CircleAvatar(
                  radius: 30,
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    const Text(
                      'Executive View',
                      style: TextStyle(color: AppConfig.secondaryColor, fontSize: 13, fontWeight: FontWeight.w600),
                    ),
                    Text(
                      user?.name ?? 'Director',
          ...
          Center(
            child: _isLoadingStats 
              ...
              : _buildSimpleStat(_totalAlumni.toString(), 'Total Alumni'),
          ),
        ],
      ),
    );
            ...
            child: Row(
              children: [
                CircleAvatar(
                  ...
                  foregroundImage: post.user.profilePicture.isNotEmpty
    ...
    return InkWell(
      ...
      child: Hero(
        ...
        child: Container(
          ...
          child: const Icon(Icons.add, color: AppConfig.primaryColor, size: 24),
        ),
      ),
    );
  }
